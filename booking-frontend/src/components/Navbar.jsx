    ...
    <header className="w-full bg-white/70 backdrop-blur border-b sticky top-0 z-20">
      <div className="mx-auto max-w-6xl px-4 h-14 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-4">
          ...
          {user?.role === "OWNER" && <Link to="/owner" className="hover:text-blue-700 transition">Owner</Link>}
          {user && <Link to="/bookings" className="hover:text-blue-700 transition">My Bookings</Link>}

          {!user ? (
            <div className="flex gap-2">
              <Link to="/login" className="px-3 py-1.5 rounded-lg border border-blue-600 text-blue-600 hover:bg-blue-50 transition">
                Login
              </Link>
              <Link to="/register" className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-[#FF6B6B] via-[#FF9154] to-[#2979FF] text-white hover:brightness-110 transition">
                Sign Up
              </Link>
            </div>
          ) : (
            <div className="relative flex items-center gap-3">
              ...
              {open && (
                <div className="absolute right-0 mt-2 top-full bg-white shadow-lg rounded-lg py-2 w-48 border border-gray-200">
                  ...
                  {user.role === "OWNER" && (
                    <Link to="/owner" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition">
                      Owner Dashboard
                    </Link>
                  )}
                  
                  <Link to="/bookings" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition">
                    My Bookings
                  </Link>
